...
export default function CompanyDirectory() {
  ...
  const yearFacet = useMemo(
    () => buildFacet(processedStartups.map((s) => s.inception_year)),
    [processedStartups]
  );
  ...
  const programmeFacet = useMemo(
    () => buildFacet(processedStartups.map((s) => s.Programme)),
    [processedStartups]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return processedStartups.filter((s) => {
      const matchesQuery = [s.Startup, s.Sector, s.Statut, s.Programme]
        ...
        .includes(q);
      const matchesYear =
        years.length === 0 || years.includes(s.inception_year);
      const matchesSector = sectors.length === 0 || sectors.includes(s.Sector);
      const matchesProgramme =
        programmes.length === 0 || programmes.includes(s.Programme);
        ...
        {!isMobile ? (
          <aside className="sticky top-24 self-start w-80 shrink-0 space-y-10 rounded-3xl bg-white/70 dark:bg-white/10 backdrop-blur-md shadow-lg p-6 border border-white/40 dark:border-white/20">
            ...
            <FacetBlock
              ...
              onToggle={(v) =>
                toggleValue(v as string, programmes, setProgrammes)
              }
            />
          </aside>
        ) : (
